...
// This is a Server Component (no "use client"), so it can query Prisma
// directly — no need for an API route + client-side fetch just to show
// some numbers. This is the same pattern used in the entrepreneur
// dashboard (app/entrepreneur/dashboard/page.tsx).
export default async function AdminDashboardPage() {
  const user = await getCurrentUser();

  // Run all the count queries at once instead of one-by-one — faster,
    ...
    <div className="flex min-h-screen bg-[#f6f8fb]">
      ...
      <div className="min-w-0 flex-1">
        ...
        <main className="p-8">
          ...
          {/* Real numbers from the database — no placeholders here */}
          <section className="mt-6 grid grid-cols-1 gap-5 md:grid-cols-2 xl:grid-cols-4">
            ...
            <div className="rounded-xl border border-slate-200 bg-white p-5">
              ...
              <p className="mt-1 text-[10px] text-slate-500">{orders.length} orders placed</p>
            </div>
