  ...
  const steps = [
    {
      ...
      title: "Apply & Verify",
      description:
        "Student status verification ensures every single product or digital service listing belongs to a registered university entrepreneur.",
    },
    {
      number: 2,
    ...
    },
    {
      ...
      description:
        "Buyers purchase securely or instantly contact sellers via WhatsApp to coordinate payments or safe library pickups.",
    },
  ];
        ...
        <section className="mx-auto grid max-w-6xl grid-cols-1 items-center gap-10 px-6 py-16 md:grid-cols-2">
          <div>
            ...
            <p className="mt-4 text-sm leading-6 text-slate-500">
              Entre Club – University of Sri Jayewardenepura is the student-led
              entrepreneurship hub established under the Entrepreneurship Development
              Unit. It serves as a platform where students with innovative ideas can
              build business concepts, develop entrepreneurial abilities, and gain
              exposure through workshops, competitions, mentoring, and networking
              opportunities with startup founders and industry professionals.
            </p>
          </div>
          <div className="flex h-full min-h-[280px] items-center justify-center rounded-xl bg-slate-100">
  <p className="text-sm text-slate-400">Photo placeholder</p>
</div>
        </section>
        ...
        <section className="bg-slate-50 py-12">
          <div className="mx-auto grid max-w-6xl grid-cols-2 gap-5 px-6 md:grid-cols-4">
            {stats.map((stat) => (
              <div key={stat.label} className="rounded-xl border border-slate-200 bg-white p-6 text-center">
        ...
        <section className="mx-auto max-w-6xl px-6 py-16 text-center">
          <h2 className="text-2xl font-bold text-slate-900">How We Spark Success</h2>
          <p className="mt-2 text-sm text-slate-500">
            Our custom campus model is designed entirely around university student schedules.
          </p>

          <div className="mt-10 grid grid-cols-1 gap-6 text-left md:grid-cols-3">
            {steps.map((step) => (
              <div key={step.number} className="rounded-xl border border-slate-200 bg-slate-50 p-6">
